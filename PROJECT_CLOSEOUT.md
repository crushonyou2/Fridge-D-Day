...
**v1.0 Released / v1.1 QA No-Go / Archived (Maintenance only)**

- v1.0은 원스토어에 출시됐다. 기존 출시 이력과 공개 상태는 유지한다.
- 출시 후 v1.1 후보에 독립 한국 식품 라벨 55장 QA와 릴리스 게이트를 적용했다.
- v1.1 후보는 기준 미달로 배포하지 않았다. 이는 프로젝트 전체의 No-Go나 “정확도가 낮아 포기”한 결과가 아니라, 측정과 회귀 기준으로 검증되지 않은 업데이트를 차단한 릴리스 판단이다.
- 추가 OCR 개선, 실사진 수집, 사용자 모집·베타, v1.1 출시, 스토어 업데이트는 하지 않는다.
- `Archived`는 문서상 개발 종료 상태다. GitHub 저장소의 Archive 설정과 원스토어 앱 상태는 변경하지 않는다.
...
- 검증 시 unit 19개, `lintDebug`, Debug/Release 빌드, AndroidTest 빌드와 instrumentation을 통과했다. 종료 변경의 `INTERNET` 권한 제거 후에도 동일 55장 D-30/D-180 회귀를 다시 실행해 각각 8분 21초·8분 3초에 기준선 대비 전체·샘플별 변화 0으로 통과했다.
...
따라서 v1.0의 출시 사실과 v1.1의 미출시 판단을 분리한다. v1.1 APK/AAB를 스토어에 제출하지 않았으며 기존 원스토어 v1.0을 업데이트하거나 비공개 전환하지 않았다.
...
- Kotlin·Jetpack Compose 기반 오프라인 우선 Android 앱을 단독 개발하고 v1.0을 원스토어에 출시했다.
...
- unit 19개, lint, Debug/Release build, instrumentation을 통과했다.
