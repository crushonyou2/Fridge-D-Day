...
> **프로젝트 상태: v1.0.2 Released (원스토어) / 활성 개발 없음**
> v1.0을 원스토어에 출시한 뒤, v1.1 후보는 독립 QA와 릴리스 게이트에서 잔여 오답을 근거로 **배포를 보류(No-Go)**했습니다.
> 이후 인식 정확도를 더 올리는 대신 **인식한 날짜를 사용자가 확인해야 저장되도록 구조를 바꿔** 잘못 읽힌 날짜가
> 저장되는 경로 자체를 제거하고, **2026-08-12에 v1.0.2를 원스토어에 배포**했습니다. 공개 배포 이력은 v1.0 → v1.0.2 2건입니다.
> 현재 추가 OCR 개선·사용자 베타·신규 기능은 계획하지 않으며, 보존에 필요한 최소 유지보수만 합니다.
...
- 원스토어 앱 출시 2건 (v1.0 → v1.0.2)
- 네트워크 권한 없이 온디바이스 데이터 처리. 원스토어가 재서명한 실제 배포 APK를 직접 검사해 `INTERNET` 권한 부재와 비공개 QA 자료 미포함을 확인
...
- Room 파괴적 마이그레이션 제거(`exportSchema = true`)와 업데이트 시 데이터 보존을 단언하는 계측 테스트
- 백업 포맷 v2 (v1 하위호환 유지)
- Android 의존성과 분리한 OCR 날짜 파서 단위 테스트 및 GitHub Actions 품질 게이트. 기본 브랜치는 PR과 CI 통과 없이 머지할 수 없습니다
- Target SDK 36, ProGuard 적용
...
이후 남은 위험을 정확도로 풀지 않고 **저장 경로에서 차단**했습니다. 인식한 날짜를 사용자가 확인해야 저장되도록 바꿔(`OcrDateConfirmationDialog`, 확정 전 저장 차단) 잘못 읽힌 날짜가 그대로 저장되는 경우를 없앴고, Room 파괴적 마이그레이션 제거와 백업 포맷 v2를 함께 넣어 **v1.0.2로 배포**했습니다(2026-08-12). 이 리팩터링 과정에서 단위 테스트는 19건에서 29건, 계측 테스트는 11건에서 15건이 됐습니다.
...
- v1.0.2의 확인 단계는 잘못 읽힌 날짜가 **저장되는** 경우를 막는 장치이며, OCR 인식 정확도 자체를 올린 것은 아닙니다.
- v1.0.2 검증은 에뮬레이터(`google_apis_playstore` API 36)에서 수행했고 실기기 검증은 하지 않았습니다.
- 신규 기능, 추가 표본 수집, OCR 정확도 개선, 사용자 모집은 활성 계획이 아닙니다. 보안·빌드 재현성 등 보존에 필요한 최소 유지보수만 예외로 둡니다.
