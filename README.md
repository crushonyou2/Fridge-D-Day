...
독립 한국 라벨 55장으로 OCR 회귀 기준선을 운영합니다. 현재 D-30 조건 정확 일치는 40/55(72.73%)이고 v1.1은 No-Go입니다. 이 값은 평가일 시나리오에 종속되므로 실사용 정확도로 일반화하지 않으며, 조건과 한계는 [OCR 벤치마크 기준](docs/qa/OCR_BENCHMARK.md)에 함께 공개합니다.
...
- OCR은 라벨 품질·촬영 환경·평가 시점에 영향을 받습니다. 로컬 55장 정량 평가셋은 회귀에 사용하지만 한글 날짜와 연속 숫자 날짜 실사진이 빠져 있어 릴리스 대표성은 아직 부족합니다.
