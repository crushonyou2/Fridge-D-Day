...
**프로젝트 상태: v1.0 Released / v1.1 QA No-Go / Archived (Maintenance only).** 아래 수치와 자동화는 출시 후 v1.1 후보를 평가해 기준 미달 배포를 차단한 종료 증거다. 추가 표본 수집, OCR 개선, 사용자 베타, v1.1 출시 또는 스토어 업데이트는 활성 계획이 아니다.
...
이 QA 작업 중 Google Play·원스토어 제출은 수행하지 않았다. 기존 원스토어 v1.0 공개 상태는 유지하며 v1.1 후보는 제출하지 않았다. 문서상 프로젝트만 Archived로 종료하고 GitHub 저장소 Archive 설정이나 스토어 상태는 변경하지 않는다.
...
- 위 한계 때문에 v1.1은 Release No-Go로 종료했다. 보완 실사진 수집, 추가 파서 변경, 재판정, 사용자 베타와 원스토어 업데이트는 계획하지 않는다.
